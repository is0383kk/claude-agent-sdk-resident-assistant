...
def cmd_start(port: int = DEFAULT_PORT) -> None:
    ...
    if status == "running":
        print(f"Casra is already running (PID: {pid})")
        return
...
def cmd_stop() -> None:
    ...
    if status == "stopped":
        print("Casra is not running.")
        return
    if status == "stale":
        PID_FILE.unlink(missing_ok=True)
        SOCKET_PATH.unlink(missing_ok=True)
        print("Casra stopped (cleaned up stale state).")
        return

    print("Stopping Casra daemon...")
    ...
    if ok:
        ...
        print("ERROR: Failed to stop Casra.", file=sys.stderr)
        sys.exit(1)
...
def cmd_status() -> None:
    ...
    status, pid = get_daemon_status()
    if status == "running":
        print(f"Casra is running (PID: {pid})")
    elif status == "stale":
        print(f"Casra has a stale PID file (PID: {pid}, process not found).")
    else:
        print("Casra is stopped.")
